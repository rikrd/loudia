...
BandStop::BandStop( int order, Real freq, Real freqStop, Real rippleDB, FilterType filterType, int channels) : 
  _order(order),
  _freq(freq),
  _freqStop(freqStop),
  _rippleDB(rippleDB),
...
{
  DEBUG("BANDSTOP: Constructor order: " << order << 
        ", freq: " << freq << 
        ", freqStop: " << freqStop <<        
        ", rippleDB: " << rippleDB );
  ...
  DEBUG("BANDSTOP: Constructed");
}

void BandStop::setup(){
  DEBUG("BANDSTOP: Setting up...");

  DEBUG("BANDSTOP: Getting zpk");  
  // Get the lowstop z, p, k
  ...
  DEBUG("BANDSTOP: poles:" << poles );
  DEBUG("BANDSTOP: gain:" << gain );
  ...
  DEBUG("BANDSTOP: Calculated the coeffs");
  ...
  DEBUG("BANDSTOP: Calculated the low pass to band stop");
  ...
  DEBUG("BANDSTOP: setup the coeffs");
  ...
  DEBUG("BANDSTOP: Finished set up...");
}

void BandStop::a(MatrixXR* a) {
  _filter.a(a);
}

void BandStop::b(MatrixXR* b) {
  _filter.b(b);
}
